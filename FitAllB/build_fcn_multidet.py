...
def FCN(inp):	
    ...
    for k in range(inp.fit['no_det']):
        ...
        for i in range(inp.no_grains):
            string = string + '\t[' 
            for j in range(0,inp.nrefl[k][i]):
                string = string + '%i,' %inp.id[k][i][j]
            string = string + '],\n'
        string = string + '\t],\n'
    string = string + '\t]\n'
    ...
    for k in range(inp.fit['no_det']):
        string = string + '[' 
        for i in range(0,inp.param['total_refl'][k]):
            nn = nn + 1
            string = string + '%f,' %inp.w[k][i]
            if nn == 20:
                nn = 0
                string = string + '\n\t'
        string = string + '],\n\t' 
    ...
    for k in range(inp.fit['no_det']):
        ...
        for i in range(inp.no_grains):
            ...
            for j in range(0,inp.nrefl[k][i]):
                nn = nn + 1
                string = string + '%s,' %inp.vars[k][i][j]
                if nn == 5:
                    nn = 0
                    string = string + '\n\t'
            string = string + '],\n'
...
# FCN function for all grains on all detectors

    string = string + 'def FCN(a,b,c,alpha,beta,gamma,wx,wy' 
    for i in range(inp.fit['no_det']):
        string = string + ',\n \t tx%s,ty%s,tz%s,py%s,pz%s,cy%s,cz%s,L%s' %(i,i,i,i,i,i,i,i)
    for i in range(inp.no_grains):
    ...
    string = string + "\t tx = ["
    for i in range(inp.fit['no_det']):
        string = string + 'tx%i,' %i
    string = string + ']\n'
    string = string + "\t ty = ["
    for i in range(inp.fit['no_det']):
        string = string + 'ty%i,' %i
    string = string + ']\n'
    string = string + "\t tz = ["
    for i in range(inp.fit['no_det']):
        string = string + 'tz%i,' %i
    string = string + ']\n'
    string = string + "\t py = ["
    for i in range(inp.fit['no_det']):
        string = string + 'py%i,' %i
    string = string + ']\n'
    string = string + "\t pz = ["
    for i in range(inp.fit['no_det']):
        string = string + 'pz%i,' %i
    string = string + ']\n'
    string = string + "\t cy = ["
    for i in range(inp.fit['no_det']):
        string = string + 'cy%i,' %i
    string = string + ']\n'
    string = string + "\t cz = ["
    for i in range(inp.fit['no_det']):
        string = string + 'cz%i,' %i
    string = string + ']\n'
    string = string + "\t L = ["
    for i in range(inp.fit['no_det']):
        string = string + 'L%i,' %i
    ...
    string = string + '\t\t\t pass \n'
    string = string + '\t\t else:\n'
    string = string + '\t\t\t for m in range(no_det):\n'
    string = string + '\t\t\t\t for j in range(nrefl[m][i]):\n'
    string = string + '\t\t\t\t\t sum = sum + peak(a,b,c,alpha,beta,gamma,h[m][i][j],k[m][i][j],l[m][i][j],w[m][id[m][i][j]],dety[m][id[m][i][j]],detz[m][id[m][i][j]],vars[m][i][j],wx,wy,' 
    string = string + '\n\t\t\t\t\t\t\t tx[m],ty[m],tz[m],py[m],pz[m],cy[m],cz[m],L[m],x[i],y[i],z[i],rod[i][0]+rodx[i],rod[i][1]+rody[i],rod[i][2]+rodz[i],epsaa[i],epsab[i],epsac[i],epsbb[i],epsbc[i],epscc[i]) \n'
    string = string + '\n'
    string = string + '\t return sum \n\n\n'

    
	
# FCN function for all grains on one detector
	
    string = string + 'def FCNonedet(a,b,c,alpha,beta,gamma,wx,wy,tx,ty,tz,py,pz,cy,cz,L' 
    for i in range(inp.no_grains):
        string = string + ',\n \t x%s,y%s,z%s,rodx%s,rody%s,rodz%s,epsaa%s,epsab%s,epsac%s,epsbb%s,epsbc%s,epscc%s' \
	                          %(i,i,i,i,i,i,i,i,i,i,i,i)
    string = string + '):\n \n'
...
# FCNgrain function for a single grain 
    ...
    string = string + 'def FCNgrain(i,a,b,c,alpha,beta,gamma,wx,wy' 
    for i in range(inp.fit['no_det']):
        string = string + ',\n \t tx%s,ty%s,tz%s,py%s,pz%s,cy%s,cz%s,L%s' %(i,i,i,i,i,i,i,i)
    for i in range(inp.no_grains):
        string = string + ',\n \t x%s,y%s,z%s,rodx%s,rody%s,rodz%s,epsaa%s,epsab%s,epsac%s,epsbb%s,epsbc%s,epscc%s' \
    ...
    string = string + '\t for m in range(no_det):\n'
    string = string + '\t\t for j in range(nrefl[m][i]):\n'
    string = string + '\t\t\t sum = sum + peak(a,b,c,alpha,beta,gamma,h[m][i][j],k[m][i][j],l[m][i][j],w[m][id[m][i][j]],dety[m][id[m][i][j]],detz[m][id[m][i][j]],vars[m][i][j], ' 
    string = string + 'wx,wy,tx[m],ty[m],tz[m],py[m],pz[m],cy[m],cz[m],L[m],x[i],y[i],z[i],rod[i][0]+rodx[i],rod[i][1]+rody[i],rod[i][2]+rodz[i],epsaa[i],epsab[i],epsac[i],epsbb[i],epsbc[i],epscc[i]) \n'
    string = string + '\n'
    string = string + '\t return sum \n\n\n'
...
# FCNpeak function calculating the contribution from a single peak including all variables	
    ...
    string = string + 'def FCNpeak(m,i,j,a,b,c,alpha,beta,gamma,wx,wy' 
    for i in range(inp.fit['no_det']):
        string = string + ',\n \t tx%s,ty%s,tz%s,py%s,pz%s,cy%s,cz%s,L%s' %(i,i,i,i,i,i,i,i)
    for i in range(inp.no_grains):
        string = string + ',\n \t x%s,y%s,z%s,rodx%s,rody%s,rodz%s,epsaa%s,epsab%s,epsac%s,epsbb%s,epsbc%s,epscc%s' \
    ...
    string = string + '\t sum = peak(a,b,c,alpha,beta,gamma,h[m][i][j],k[m][i][j],l[m][i][j],w[m][id[m][i][j]],dety[m][id[m][i][j]],detz[m][id[m][i][j]],vars[m][i][j], ' 
    string = string + 'wx,wy,tx[m],ty[m],tz[m],py[m],pz[m],cy[m],cz[m],L[m],x[i],y[i],z[i],rod[i][0]+rodx[i],rod[i][1]+rody[i],rod[i][2]+rodz[i],epsaa[i],epsab[i],epsac[i],epsbb[i],epsbc[i],epscc[i]) \n'
    string = string + '\t return sum \n\n\n'
